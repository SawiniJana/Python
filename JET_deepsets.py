...
class Jet_Dataset(data.Dataset):
    ...
    def pt_cloud(self, idx):
        ...
        return {'jet': con, 'label': torch.tensor([jet_class]), 
...
class DeepSetModule(nn.Module):
    ...
    def forward(self, x): #(N, feats)
        
        for conv in self.convs:
            x = conv(x) #(N,16)
        
        x = x.mean(dim=1) #(N,1)
        out = F.log_softmax(x, dim=-1) #then we compute NLL Loss
...
valid_loader = data.DataLoader(jet_dataset, batch_size = batch_size, sampler = valid_sampler, collate_fn=collate_fn)
